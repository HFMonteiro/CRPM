...
def collect_timing_buckets(log: EventLog) -> Dict[str, Any]:
    """Collect reusable timing buckets from one pass through the log."""
    ...
    for trace in log:
        ...
        for i, event in enumerate(trace):
            ...
            activity_durations.setdefault(activity, []).append(duration)
            transition_durations.setdefault((activity, next_activity), []).append(duration)
...
def compute_activity_statistics(log: EventLog, timing_buckets: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    ...
    """
    activity_data = []
    buckets = timing_buckets if timing_buckets is not None else collect_timing_buckets(log)
    activity_counts = buckets.get("activity_counts", {})
    activity_durations = buckets.get("activity_durations", {})
...
def compute_transition_statistics(
    ...
    timing_buckets: Optional[Dict[str, Any]] = None,
) -> pd.DataFrame:
    ...
    """
    buckets = timing_buckets if timing_buckets is not None else collect_timing_buckets(log)
    transition_data: Dict[Tuple[str, str], List[float]] = buckets.get("transition_durations", {})

    # Compute statistics
    results = []
    for (activity, next_activity), durations in transition_data.items():
        if model_transitions is not None and (activity, next_activity) not in model_transitions:
            continue
...
def compute_case_durations(log: EventLog, timing_buckets: Optional[Dict[str, Any]] = None) -> pd.DataFrame:
    ...
    """
    if timing_buckets is not None and "case_durations" in timing_buckets:
        case_data = list(timing_buckets.get("case_durations", []))
        if case_data:
