...
class PreflightReport:
    ...
    def ok(self) -> bool:
        return not (
            ...
            or self.graphviz_executable is None
            or self.graphviz_error is not None
            or self.missing_sample_files
        )
...
def validate_runtime_environment(*, repo_root: Path | None = None) -> PreflightReport:
    ...
    missing_python_modules = tuple(
        module_name for module_name in REQUIRED_PYTHON_MODULES if not _module_exists(module_name)
    )
    graphviz_executable, graphviz_version, graphviz_error = _check_graphviz_binary()
    missing_sample_files = tuple(
        str(path)
...
def format_preflight_messages(report: PreflightReport) -> list[str]:
    ...
    if report.missing_python_modules:
        ...
        lines.append(
            "Install the package with `pip install -e \".[dev]\"` for development or `pip install .` for runtime."
        )
    if report.graphviz_executable is None:
        lines.append("Graphviz `dot` was not found on `PATH`.")
        lines.append(
            "Install Graphviz from https://graphviz.org/download/ and ensure `dot` is available before launching CRPM."
        )
